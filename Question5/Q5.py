...
class RomaniaMap:
    def __init__(self):
        # Graph representation of Romania's cities and their distances
        self.graph = {
    ...
    def bfs(self, start, goal):
        ...
        visited = set()
        q = deque([(start, [start], 0)])  # Queue storing (current city, path, cost)
        
        while q:
            current, path, cost = q.popleft()
            
            if current == goal:
                return path, cost  # Return path and total cost if goal is reached
    ...
    def ucs(self, start, goal):
        ...
        visited = set()
        pq = [(0, start, [start])]  # Priority queue storing (cost, current city, path)
    ...
    def gbfs(self, start, goal):
        ...
        visited = set()
        pq = [(self.heuristic[start], start, [start], 0)]  # Priority queue sorted by heuristic value
        ...
        while pq:
            ...
            if current not in visited:
                ...
                for neighbor in self.graph[current]:
                    if neighbor not in visited:
                        new_cost = cost + self.graph[current][neighbor]
                        heapq.heappush(pq, (self.heuristic[neighbor], neighbor, path + [neighbor], new_cost))
    ...
    def iddfs(self, start, goal):
        ...
        def dfs(node, goal, depth, path, cost):
            ...
            for neighbor in self.graph[node]:
                ...
                result, final_cost = dfs(neighbor, goal, depth - 1, path + [neighbor], new_cost)
                if result is not None:
                    return result, final_cost
            return None, 0

        for depth in range(50):  # Limiting depth to prevent infinite recursion
